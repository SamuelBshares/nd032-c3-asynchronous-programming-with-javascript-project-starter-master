...
async function runRace(raceID) {
  return new Promise((resolve) => {
    const racerInterval = setInterval(async () => {
      const getRaceRes = await getRace(raceID).catch((error) =>
        console.log("Error on getRace", error)
      );

      if (getRaceRes.status === "in-progress") {
        renderAt("#leaderBoard", raceProgress(getRaceRes.positions));
      } else if (getRaceRes.status === "finished") {
        clearInterval(racerInterval);
        renderAt("#race", resultsView(getRaceRes.positions));
        resolve(getRaceRes);
      }
    }, 500);
